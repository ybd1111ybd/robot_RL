...
print(f"[INFO] JZ training task={args_cli.task!r} hydra_overrides={hydra_args!r}", flush=True)
...
def main(env_cfg: "ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg", agent_cfg: dict) -> None:
    print("[INFO] Entered JZ RL-Games training main().", flush=True)
...
if __name__ == "__main__":
    try:
        try:
            ...
            print(f"[ERROR] JZ training exited before runner start: {type(exc).__name__}: {exc!r}", flush=True)
            traceback.print_exc()
            raise
    finally:
